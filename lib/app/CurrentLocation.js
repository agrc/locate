define([
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
        ...
        onFindAddress: function (data) {
            ...
            this.onMapClick(data[0].location, data[0].standardizedAddress);
        },
        ...
        onMapClick: function (point, geocodedAddress) {
            ...
            // if no point (sure site was clicked) then clear out data
            if (!point) {
                domClass.add(this.addressContainer, 'hidden');
                domClass.remove(this.helpTxt, 'hidden');
                return;
            }
        ...
        toggleMobilePane() {
            // summary:
            //      description
            console.log('app/CurrentLocation:toggleMobilePane', arguments);

            topic.publish(config.topics.toggleMobilePane);
        }
    });
});
