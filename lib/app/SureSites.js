define([
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
        ...
        toggleEnableControls() {
            ...
            // filter controls
            const enableFilter = enableRadioButtons && this.filterRB.checked;

            query('.indent input[type="checkbox"]', this.domNode).attr('disabled', !enableFilter);

            const sliderMethod = (enableFilter) ? 'enable' : 'disable';
            $(this.acreageSlider).slider(sliderMethod);
            $(this.sqftSlider).slider(sliderMethod);
        }
    });
});
